...
def seller_signup(request):
    if request.method == "POST":
        ...
        print(
            f"Username: {owner_name}, Email: {email}, Password: {password}, Confirm Password: {confirm_password}, Phone: {phone}"
        )
...
def add_product(request):
    ...
    if request.session.get("seller_id"):
        ...
        return render(
            ...
            {"seller": seller, "categories": categories},
        )
    else:
...
def search_view(request):
    ...
    return render(
        request, "search_results.html", {"products": products, "query": query}
    )
